...
handlers.move = function(args)
  ...
  -- Send status update after movement (position, fuel, etc.)
...
handlers.moveTo = function(args)
  ...
  if not machine.moveDestination then
    print("ERROR: Failed to set machine.moveDestination!")
    return false
  end
  ...
  print("DEBUG: machine.moveDestination set to:", textutils.serialize(machine.moveDestination))
  print("DEBUG: machine.moveDestination ~= nil:", machine.moveDestination ~= nil)
  print("DEBUG: machine.moveDestination.x:", machine.moveDestination.x, "y:", machine.moveDestination.y, "z:", machine.moveDestination.z)
  ...
  if ok and result then
    ...
    print("DEBUG: Before sendStatus() - machine.currentStatus:", machine.currentStatus, ", machine.moveDestination ~= nil:", machine.moveDestination ~= nil)
    if machine.moveDestination then
      print("DEBUG: machine.moveDestination still exists:", textutils.serialize(machine.moveDestination))
    else
      print("DEBUG: ERROR - machine.moveDestination is nil before sendStatus()!")
    end
    if sendStatus then sendStatus() end  -- Send status update immediately
    print("DEBUG: After sendStatus() - machine.currentStatus:", machine.currentStatus, ", machine.moveDestination ~= nil:", machine.moveDestination ~= nil)
    return true
  else
...
handlers.turn = function(args)
  ...
  -- Send status update after turning (facing direction)
  if sendStatus then sendStatus() end
end
...
handlers.update = function(args)
  ...
  if machine.isTurtle then
    ...
    if machine.location and machine.location ~= "unknown" and type(machine.location) == "table" then
      ...
      elseif ok and data and isMachineBlock(data.name) then
        print("Skipped machine block in front:", data.name)
      end
      ...
      elseif ok and data and isMachineBlock(data.name) then
        print("Skipped machine block above:", data.name)
      end
      ...
      elseif ok and data and isMachineBlock(data.name) then
        print("Skipped machine block below:", data.name)
      end
